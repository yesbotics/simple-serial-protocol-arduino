#include "Arduino.h"
#include "SimpleSoftwareSerialProtocol.h"
#include <stdarg.h>
...
SimpleSoftwareSerialProtocol::SimpleSoftwareSerialProtocol()
{
}

void SimpleSoftwareSerialProtocol::initialize(ErrorFunctionPointer errorFctPtr, SoftwareSerial *serialPtr, long baudrate, unsigned long timeout)
{
...
void SimpleSoftwareSerialProtocol::registerCommand(const uint8_t command, FunctionPointer fctPtr, const uint8_t numArguments)
{
...
void SimpleSoftwareSerialProtocol::onError(const uint8_t errorNum, const uint8_t command, const uint8_t numValues)
{
...
void SimpleSoftwareSerialProtocol::receive()
{
...
void SimpleSoftwareSerialProtocol::flush()
{
...
void SimpleSoftwareSerialProtocol::countArguments()
{
	for (uint8_t i = 0; i < _count; i++)
	{
		uint8_t byte = _buffer[i];

		if (byte == SS_BYTE_DELIMITER)
		{
			_numValues++;
		}
	}
...
void SimpleSoftwareSerialProtocol::handleCommand(const bool msgComplete)
{
	if (!msgComplete)
	{
...
String SimpleSoftwareSerialProtocol::getMessage()
{
	char txt[_count + 2];
...
String SimpleSoftwareSerialProtocol::getStringValue()
{
...
int SimpleSoftwareSerialProtocol::getIntValue()
{
	return getStringValue().toInt();
}

float SimpleSoftwareSerialProtocol::getFloatValue()
{
...
bool SimpleSoftwareSerialProtocol::getBoolValue()
{
	return getIntValue()==1;
}

void SimpleSoftwareSerialProtocol::send(String msg)
{
	_serialPointer->print(msg);
}

void SimpleSoftwareSerialProtocol::sendln(String msg)
{
